...
exports.update = function(request, response) {
    ...
    db.query('SELECT * FROM topic', function(error, topics) {
        ...
        db.query('SELECT * FROM author WHERE id=?', [queryData.id], function(err, author) {
            ...
            db.query('SELECT * FROM author', function(e, authors) {
                var list = template.list(topics);
                var html = template.HTML(author[0].name, list,
                    `
                    ${template.authorTable(authors)}
                    <style>
                        table{
...
exports.delete_process = function(request, response) {
    ...
    request.on('end', function(){
        var post = qs.parse(body);
        db.query('DELETE FROM topic WHERE author_id=?', [post.id], function(error, results) {
            if (error) {
                throw error;
            };
